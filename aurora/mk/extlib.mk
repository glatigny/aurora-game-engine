...
aoe.mk: Makefile
$(MK_PATH)/extlib.mk: aoe.mk
$(MK_PATH)/pspec.mk: $(MK_PATH)/extlib.mk
$(MK_PATH)/common.mk: $(MK_PATH)/extlib.mk

# Update the config makefile
CONFIG_MAKEFILE = $(MK_PATH)/config.mk
$(CONFIG_MAKEFILE):: $(MK_PATH)/extlib.mk aoe.mk
	@echo " [CUSTOM] $(notdir $@)"
	@rm -f $@
...
$(FACTORY_EXTENSION): $(MK_PATH)/extlib.mk Makefile
	@echo " [CUSTOM] $(notdir $@)"
	@rm -f $@
	@touch $@
	@for f in $(addsuffix .h,$(basename $(SRC))); do [ -e "src/$${f%.h}.xh" ] && echo "#include \"$$f\"" >> $@ || true ; done
	
$(XH_EXTENSION): $(MK_PATH)/extlib.mk Makefile
	@echo " [CUSTOM] $(notdir $@)"
	@rm -f $@
	@touch $@
